...
export default function SitesList() {
  ...
  useEffect(() => {
    // Internal QA must always land on the site selector, even when only one
    // client exists, so the tester is explicitly choosing the tenant under test.
  ...
  if (!loadingMe && me !== null && me?.isActive === false) {
    return <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4"><div className="max-w-md w-full rounded-2xl border border-red-100 bg-white p-8 text-center shadow-xl"><div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-red-100"><ShieldX className="h-7 w-7 text-red-600" /></div><h1 className="text-2xl font-bold text-slate-950 mb-2">Account Deactivated</h1><p className="text-slate-500 text-sm mb-6">Your dashboard access is currently disabled. Please contact your FSTS administrator for assistance.</p><Button variant="outline" className="w-full" onClick={() => signOut({ redirectUrl: window.location.origin })}><LogOut className="h-4 w-4 mr-2" />Sign Out</Button></div></div>;
  }

  const workspaceTitle = me?.isSuperAdmin ? "Website Operations" : isInternalQa ? "FSTS QA Workspaces" : "Your Website Workspace";
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="sticky top-0 z-30 border-b border-slate-800/80 bg-slate-950 text-white shadow-sm">
        <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-white/5"><LayoutDashboard className="h-4 w-4 text-lime-300" /></div><div><div className="text-sm font-semibold">FSTS Website Operating System™</div><div className="hidden text-[11px] text-slate-400 sm:block">Client management workspace</div></div></div>
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="hidden items-center gap-2 rounded-lg border border-slate-800 bg-slate-900 px-3 py-1.5 text-sm md:flex">{loadingMe ? <Skeleton className="h-4 w-24 bg-slate-700" /> : <><span className="max-w-[180px] truncate font-medium text-white">{me?.name || "Account"}</span><span className="h-3 w-px bg-slate-700" /><span className={`text-[10px] font-semibold uppercase tracking-[0.14em] ${isInternalQa ? "text-amber-300" : "text-slate-400"}`}>{me?.isSuperAdmin ? "FSTS Admin" : isInternalQa ? "FSTS Internal QA" : "Client"}</span></>}</div>
            <Button variant="ghost" size="sm" className="text-slate-300 hover:bg-slate-900 hover:text-white" onClick={() => signOut({ redirectUrl: import.meta.env.BASE_URL || "/" })}><LogOut className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Sign Out</span></Button>
          </div>
        </div>
      </header>

      {isInternalQa && <div className="border-b border-amber-300 bg-amber-50"><div className="mx-auto flex w-full max-w-7xl items-center gap-3 px-4 py-3 text-sm text-amber-950 sm:px-6 lg:px-8"><FlaskConical className="h-4 w-4 flex-shrink-0" /><strong>FSTS Internal QA Mode</strong><span className="text-amber-800">Client workspaces are available for controlled testing and support. Your activity remains under your FSTS identity.</span></div></div>}

      <main className="flex-1">
        <section className="border-b border-slate-200 bg-white"><div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-2xl"><div className={`mb-3 inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold ${isInternalQa ? "border-amber-200 bg-amber-50 text-amber-800" : "border-lime-200 bg-lime-50 text-lime-800"}`}>{isInternalQa ? <FlaskConical className="h-3.5 w-3.5" /> : <Sparkles className="h-3.5 w-3.5" />}{isInternalQa ? "Internal testing & support" : "Website management made simple"}</div><h1 className="text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">{workspaceTitle}</h1><p className="mt-3 text-sm leading-6 text-slate-500 sm:text-base">{workspaceDescription}</p></div>{me?.isSuperAdmin && <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setLocation("/app/admin/sites")}><Settings className="h-4 w-4 mr-2" />Platform Settings</Button><Button onClick={() => setLocation("/app/onboard")}><Rocket className="h-4 w-4 mr-2" />Onboard New Site</Button></div>}</div></div></section>

        <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          {me?.isSuperAdmin && <section className="mb-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4"><h2 className="text-sm font-semibold text-slate-900">FSTS Administration</h2><p className="mt-1 text-xs text-slate-500">Platform-only controls are separated from client website workspaces.</p></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Button variant="outline" onClick={() => setLocation("/app/admin/users")}><Users className="h-4 w-4 mr-2" />Users</Button><Button variant="outline" onClick={() => setLocation("/app/admin/access-control")}><ShieldCheck className="h-4 w-4 mr-2" />Access Control</Button><Button variant="outline" onClick={() => setLocation("/app/admin/roles")}><KeyRound className="h-4 w-4 mr-2" />Roles</Button><Button variant="outline" onClick={() => setLocation("/app/admin/design-lock")}><ShieldCheck className="h-4 w-4 mr-2" />Design Lock™</Button><Button variant="outline" onClick={() => setLocation("/app/admin/agencies")}><Landmark className="h-4 w-4 mr-2" />Agencies</Button><Button variant="outline" onClick={() => setLocation("/app/admin/platform-controls")}><SlidersHorizontal className="h-4 w-4 mr-2" />Platform Controls</Button><Button variant="outline" onClick={() => setLocation("/app/admin/runbook")}><BookOpen className="h-4 w-4 mr-2" />Runbook</Button><Button variant="outline" onClick={() => setLocation("/app/admin/sites")}><Settings className="h-4 w-4 mr-2" />Site Administration</Button></div></section>}
