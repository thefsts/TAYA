...
export const seedEvents = internalMutation({
  ...
  handler: async (ctx, { siteId }) => {
    ...
    function ctMs(isoDate: string, hour = 9): number {
      // Approximate Central Time (UTC-5 in CDT, UTC-6 in CST).
      // Using UTC-5 for summer/fall 2026 dates (CDT).
      return new Date(`${isoDate}T${String(hour).padStart(2, "0")}:00:00-05:00`).getTime();
    }
